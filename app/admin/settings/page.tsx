...
export default async function AdminSettingsPage() {
  ...
  const settings = isFounder ? await prisma.foundationSettings.findFirst() : null;
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {isFounder ? (
        ...
        <div className="card p-7">
          <p className="text-sm text-charcoal-soft flex items-start gap-2">
            <Lock className="w-4 h-4 mt-0.5 text-bark shrink-0" />
            Foundation details (public contact information, footer text and social links) can only be changed by the
            founder account. Ask the founder if something needs updating.
          </p>
        </div>
      )}
